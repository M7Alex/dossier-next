...
// ── Typewriter hook — retourne texte partiel + curseur ──
...
// ── Typewriter pour HTML (affichage séquentiel de blocs) ──
// Retourne un multiplicateur de visibilité selon le stagger
function useBlockReveal(active: boolean, blockIndex: number, totalBlocks: number) {
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    if (!active) { setVisible(true); return; }
    setVisible(false);
    const delay = 180 + blockIndex * 320; // stagger entre blocs
...
function E({ sk, html, className, style, tag = 'div' }: {
  sk: string; html: string; className?: string; style?: React.CSSProperties; tag?: string;
}) {
  ...
  return (
    <Tag ref={ref} contentEditable={editable} suppressContentEditableWarning
      onBlur={(e: any) => setContent(sk, (e.target as HTMLElement).innerHTML)}
      className={className} style={{ ...style, outline: 'none' }}
      dangerouslySetInnerHTML={undefined}/>
  );
}

// ── Block avec reveal animé + typewriter sur le titre ──
function Block({
  titre, children, delay = 0, flex = false, isActive = true, blockIndex = 0,
}: {
  titre: string; children: React.ReactNode; delay?: number;
  flex?: boolean; isActive?: boolean; blockIndex?: number;
}) {
  const ref     = useRef<HTMLDivElement>(null);
  const inView  = useInView(ref, { once: true });
  const visible = useBlockReveal(isActive, blockIndex, 6);
  const { out: twTitre, done: twDone } = useTypewriter(titre, isActive && visible, 28, 0);

  return (
    <motion.div ref={ref}
      initial={{ opacity: 0, y: 18 }}
      animate={inView && visible ? { opacity: 1, y: 0 } : {}}
      ...
      <div style={{
        fontFamily: 'Cinzel,serif', fontSize: 11, color: G,
        letterSpacing: 2, textTransform: 'uppercase',
        marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8,
      }}>
        <span>
          {isActive ? twTitre : titre}
          {!twDone && isActive && visible && (
            <span style={{ opacity: 0.6, color: G, animation: 'blink .7s infinite' }}>|</span>
          )}
        </span>
...
const BX = {
  fontSize: 17, lineHeight: 1.78, color: '#F0E8D0',
  fontWeight: 300, fontFamily: 'Raleway,sans-serif',
};
const LI_STYLE = {
  padding: '9px 0 9px 22px', position: 'relative' as const,
  fontSize: 16, color: '#F0E8D0', fontWeight: 300,
  borderBottom: '1px solid rgba(201,168,76,0.045)',
  lineHeight: 1.55, fontFamily: 'Raleway,sans-serif',
};
...
export default function ChapterSlide({ slide, isActive }: { slide: any; isActive: boolean }) {
  ...
  const { out: twTitle, done: twDone } = useTypewriter(rawTitle, isActive, 28, 120);

  // Scramble effect sur le numéro de chapitre
  const [scramble, setScramble] = useState('?');
  const CHARS = '0123456789ABCDEF▓░';
  useEffect(() => {
    if (!isActive) { setScramble(String(slide.numero)); return; }
    let count = 0;
    const target = String(slide.numero);
    const iv = setInterval(() => {
      count++;
      if (count > 12) { setScramble(target); clearInterval(iv); return; }
      setScramble(CHARS[Math.floor(Math.random() * CHARS.length)]);
    }, 55);
    return () => clearInterval(iv);
  }, [isActive, slide.numero]);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: 'transparent' }}>
      ...
      {/* Header */}
      <div style={{
        position: 'relative', zIndex: 3, height: 185,
        display: 'flex', flexDirection: 'column', justifyContent: 'center',
        padding: '0 100px', borderBottom: '1px solid rgba(201,168,76,0.1)',
      }}>
        <motion.div
          initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.05 }}
          style={{
            fontFamily: 'Cinzel,serif', fontSize: 12, color: '#8B6914',
            letterSpacing: 4, textTransform: 'uppercase', marginBottom: 10,
            display: 'flex', alignItems: 'center', gap: 10,
          }}>
          {/* Numéro scramble */}
          <span style={{ fontFamily: 'Share Tech Mono,monospace', color: G, minWidth: 18 }}>
            {scramble}
          </span>
          <span style={{ opacity: 0.4 }}>/</span>
          <span>Chapitre {slide.numero}</span>
        </motion.div>

        {/* Titre typewriter */}
        <div style={{
          fontFamily: 'Cinzel,serif', fontSize: 42, color: G,
          fontWeight: 600, textTransform: 'uppercase', letterSpacing: 2,
          minHeight: 52,
        }}>
          {isActive ? twTitle : rawTitle}
          {!twDone && isActive && (
            <span style={{ opacity: 0.7, color: G, animation: 'blink .85s infinite' }}>|</span>
          )}
        </div>

        {/* Ligne de scan animée sous le header */}
        {isActive && (
          <motion.div
            ...
            style={{
              position: 'absolute', bottom: 0, left: 100, right: 100, height: 1,
              background: `linear-gradient(90deg,transparent,${G},rgba(201,168,76,0.2))`,
            }}
          />
        )}
        ...
        <div style={{
          ...
          transform: 'translateY(-50%)', fontSize: 60,
      ...
      <div style={{
        ...
        padding: '40px 100px', height: 'calc(100% - 185px)', overflow: 'hidden',
      }}>
        {slide.id === 'intro'      && <IntroContent c={c} editable={editable} isActive={isActive}/>}
        {slide.id === 'parcours'   && <ParcoursContent c={c} editable={editable} isActive={isActive}/>}
        {slide.id === 'vision'     && <VisionContent c={c} editable={editable} isActive={isActive}/>}
        {slide.id === 'systemes'   && <SystemesContent c={c} editable={editable} isActive={isActive}/>}
        {slide.id === 'leadership' && <LeadershipContent c={c} editable={editable} isActive={isActive}/>}
        {slide.id === 'conclusion' && <ConclusionContent c={c} editable={editable} isActive={isActive}/>}
        {slide.id === 'custom'     && <CustomContent c={c} editable={editable} id={slide.id} isActive={isActive}/>}
      </div>
    </div>
  );
}

function IntroContent({ c, editable, isActive }: any) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, height: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <Block titre="Positionnement" delay={0} isActive={isActive} blockIndex={0}>
          <E sk="intro_pos" html={c.blocs[0].texte.replace(/\n\n/g,'<br/><br/>')} style={BX}/>
        </Block>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <Block titre="Contexte & Ambition" delay={1} isActive={isActive} blockIndex={1}>
          <E sk="intro_ctx" html={c.blocs[1].texte.replace(/\n\n/g,'<br/><br/>')} style={BX}/>
        </Block>
        <Block titre="Engagement" delay={2} isActive={isActive} blockIndex={2}>
...
function ParcoursContent({ c, editable, isActive }: any) {
  const { content, setContent } = useDossier();
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, height: '100%' }}>
      <Block titre="Trajectoire Professionnelle" delay={0} flex isActive={isActive} blockIndex={0}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <Block titre="Chiffres Clés" delay={1} isActive={isActive} blockIndex={1}>
        ...
        <Block titre="Compétences" delay={2} flex isActive={isActive} blockIndex={2}>
...
function VisionContent({ c, editable, isActive }: any) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, height: '100%' }}>
      <Block titre="Objectif Stratégique" delay={0} isActive={isActive} blockIndex={0}>
        <E sk="vision_obj" html={c.objectif} style={BX}/>
      </Block>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, flex: 1 }}>
        <Block titre="Axes de Développement" delay={1} isActive={isActive} blockIndex={1}>
          <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
        ...
        <Block titre="Profil de Compétences" delay={2} isActive={isActive} blockIndex={2}>
...
function SystemesContent({ c, editable, isActive }: any) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, height: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <Block titre="Diagnostic d'Entreprise" delay={0} isActive={isActive} blockIndex={0}>
          <E sk="sys_diag" html={c.diagnostic} style={BX}/>
        </Block>
        <Block titre="Segmentation Clients" delay={1} isActive={isActive} blockIndex={1}>
      ...
      <Block titre="Systèmes Déployés" delay={2} flex isActive={isActive} blockIndex={2}>
        <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
...
function LeadershipContent({ c, editable, isActive }: any) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, height: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <Block titre="Structure de l'Équipe" delay={0} isActive={isActive} blockIndex={0}>
          <E sk="lead_struct" html={c.structure} style={BX}/>
        </Block>
        <Block titre="Philosophie" delay={1} isActive={isActive} blockIndex={1}>
          <E sk="lead_phil" html={c.philosophie} style={BX}/>
        </Block>
      </div>
      <Block titre="Standards & Méthodes" delay={2} flex isActive={isActive} blockIndex={2}>
        <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
...
function ConclusionContent({ c, editable, isActive }: any) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, height: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <Block titre="Impact sur le Serveur" delay={0} isActive={isActive} blockIndex={0}>
          <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
        ...
        <Block titre="Conclusion" delay={1} isActive={isActive} blockIndex={1}>
...
function CustomContent({ c, editable, id, isActive }: any) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, height: '100%' }}>
      {c.blocs?.map((b: any, i: number) => (
        <Block key={b.key} titre={b.titre} delay={i} isActive={isActive} blockIndex={i}>
